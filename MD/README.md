...
> **현재 데이터 기준:** 타워 108종(기본/연쇄/독/골드 각 27종), 스킬 14종, 52라운드입니다.
> 능력치·웨이브·가챠 활성 여부의 단일 진실 소스는 `assets/data/stats.json`입니다.
...
| 가챠 비용 | 100G / 회 |
| 총 라운드 | 52 |
| 보스 라운드 | R8, R16, R24, R32, R40, R48, R52 |
...
### 데이터 에디터
```
http://localhost:3000/tools/data-editor.html
```
...
stats.json (데이터 소스)
    ...
    └── monsters{}     ← 몬스터 52라운드 정의 (HP, 타입, 보스여부)
...
- 모든 빈 슬롯의 실제 경로 커버리지를 비교해 자동 배치합니다. 등급점수는 배치에 사용하지 않습니다.
- 바깥 링은 리스폰 직후를 빠르게 공격하도록 11~12시 사이의 `(4,0)`부터 반시계 방향으로 감쇠형 우선 보정을 적용합니다.
- 이 보정은 바깥 링에만 적용하며, 기본 점수는 실제 몬스터 이동 중심선과 사거리 원이 겹치는 경로 길이입니다.
...
| **연쇄_연 타워** | 27종 | ✅ 활성 | 4번 (연쇄 번개) | 3공격타입 × 9등급 |
| **독_dok 타워** | 27종 | ✅ 활성 | 5번 (맹독 중첩) | 3공격타입 × 9등급, 치명타 0% |
| **골드_don 타워** | 27종 | ✅ 활성 | 6~14번 (골드 획득) | 3공격타입 × 9등급 |
...
### 일반 타워 목록 (27종, 능력치 스냅샷)

> 아래 표는 문서 갱신 시점의 참고용입니다. 밸런스 작업에는 항상 `stats.json`의 현재 값을 사용하세요.
...
### 독 타워 (_dok, 폭발 타입 예시 9종) — 현재 뽑기 활성

| ID | 이름 | 등급 | 공격력 | 공속(ms) | 특징 |
|---|---|---|---|---|---|
| n2_dok | 폭파병_독 | normal | 10 | 2500 | 치명타 0%, 맹독 20스택 |
| r2_dok | 포병_독 | rare | 17 | 2375 | 치명타 0%, 맹독 20스택 |
| a2_dok | 화염포_독 | ancient | 38 | 2250 | 치명타 0%, 맹독 20스택 |
| e2_dok | 포격사_독 | relic | 63 | 2125 | 치명타 0%, 맹독 20스택 |
| s2_dok | 파괴자_독 | saga | 375 | 2000 | 치명타 0%, 맹독 20스택 |
| l2_dok | 전설 포격_독 | legend | 485 | 1875 | 치명타 0%, 맹독 20스택 |
| ep2_dok | 에픽 파괴_독 | epic | 968 | 1750 | 치명타 0%, 맹독 20스택 |
| m2_dok | 신화 파괴_독 | myth | 1,913 | 1625 | 치명타 0%, 맹독 20스택 |
| p2_dok | 태초의 파괴_독 | primordial | 6,410 | 1500 | 치명타 0%, 맹독 20스택 |

> 독 타워는 공격 타입별로 9등급씩, 총 27종이 정의되어 있습니다.
> 실제 공격력과 독 파라미터는 `stats.json`을 기준으로 확인합니다.
...
라운드 배율은 R1부터 R52까지 적용합니다. 몬스터 HP와 골드의 현재 적용 공식은
`AGENTS.md`의 라운드 배율 정의를 따릅니다. 최대 스폰 수량 제한,
...
개별 몬스터 능력치, 스폰 수량, 간격, 시간제한, 클리어 보상은 모두
`stats.json`의 `monsters`와 `waves`에 저장됩니다. 밸런스 변경 시 과거의 선형 조정
기록이 아니라 현재 적용 데이터를 기준값으로 사용합니다.
...
| 라운드 | 보스 타입 | HP | 색상 |
|---|---|---|---|
| R8 | boss_small (소형) | 12,023 | 🟠 주황 |
| R16 | boss_normal (일반) | 38,751 | 🔴 빨강 |
| R24 | boss_normal (일반) | 98,368 | 🔴 빨강 |
| R32 | boss_large (대형) | 232,163 | 🔵 파랑 |
| R40 | boss_normal (일반) | 354,748 | 🔴 빨강 |
| R48 | boss_small (소형) | 453,761 | 🟠 주황 |
| R52 | boss_large (대형) | 620,936 | 🔵 파랑 |
...
> 🔵 = 저항 (해당 공격이 적게 들어옴)  
> 보스_대형은 진동 타워에 강하며, 보스_소형은 폭발 타워에 강합니다.
...
| 파라미터 | 값 | 설명 |
|---|---|---|
| poisonDuration | 15,000ms | 독 지속 시간 (15초) |
...
| 몬스터 | 52라운드 1마리 기준 HP·골드, 수량, T_HP, T_GOLD, 타입/보스여부 |
| 스테이지 | 웨이브 구성 |
