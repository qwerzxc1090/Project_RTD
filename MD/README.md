...
> **현재 데이터 기준:** 타워 108종(기본/연쇄/독/골드 각 27종), 스킬 14종, 50라운드입니다.
> 아래의 개별 능력치 표 일부는 이전 밸런스 스냅샷이므로 실제 수치는
> `assets/data/stats.json`과 `tools/data-editor.html`을 기준으로 확인하세요.
...
| 라운드 클리어 보너스 | `waves[].clearGoldBonus` |
...
Node.js가 설치되어 있다면 `node server.js`로 실행한 뒤
`http://localhost:3000`으로 접속할 수도 있습니다.
...
```
stats.json (데이터 소스)
    ├── units[]        ← 타워 108종 정의 (능력치 + skillId)
    ├── skills{}       ← 스킬 14종 정의 (발사체 동작 + category)
...
### 타워 배치
- 경로 안쪽의 11×11 그리드 슬롯에 자동 배치하며 슬롯 크기는 34px입니다.
- 사거리 180 이상인 타워만 예약된 모서리 영역을 사용합니다. 등급점수는 모서리 판정에 사용하지 않습니다.
- 사거리 160 미만 단거리와 160~179 중거리 타워는 동일한 바깥 링 우선 규칙을 사용합니다.
- 장거리·모서리 대상 타워는 각 모서리 최외곽 3슬롯(총 12슬롯)을 우선 사용합니다. 단, 최적 모서리 대비 경로 커버리지가 85% 미만이면 실제 효율이 높은 안쪽 모서리를 선택합니다.
- 단거리·중거리 타워는 바깥 링의 `(2,0) → (3,0)`에서 시작해 10시·7시·5시·1시 모서리를 연결하며 배치한 뒤, 두 번째 링의 12시부터 반시계 방향으로 진행합니다.
- 중앙과 중앙 아래 `(5,6)` 슬롯의 경로 커버리지가 95% 이내로 비슷하면 중앙 슬롯을 우선 사용합니다.
- 각 꼭짓점과 꼭짓점 양쪽의 인접 슬롯 2개를 모두 모서리로 판정합니다.
- 그 외 타워는 좌측 상단 기준 `(3,0)` 슬롯부터 반시계 방향으로 배치하며 모서리를 건너뜁니다.
- 일반 슬롯은 한 자리에 타워 4개를 배치한 뒤 다음 슬롯으로 이동합니다.
- 배치 가능 여부와 효율은 경로 테두리가 아닌 실제 몬스터 이동 중심선을 기준으로 계산합니다.
- 모서리 점유 패널티는 적용하지 않으며, 중앙 슬롯은 10개 전용 원형 좌표를 사용합니다.
- 각 슬롯의 4등분 중심을 기준으로 사분면 폭의 ±30%(34px 슬롯 기준 ±5.1px) 무작위 지터를 적용합니다.
...
| 그룹 | 수 | 뽑기 가능 | 스킬 | 특징 |
|---|---|---|---|---|
| **일반 타워** | 27종 | ✅ 활성 | 1~3번 (즉시) | 3공격타입 × 9등급 |
| **연쇄_연 타워** | 27종 | ❌ 비활성 | 4번 (연쇄 번개) | 3공격타입 × 9등급 |
| **독_dok 타워** | 27종 | ❌ 비활성 | 5번 (맹독 중첩) | 3공격타입 × 9등급, 치명타 0% |
| **골드_don 타워** | 27종 | ❌ 비활성 | 6~14번 (골드 획득) | 3공격타입 × 9등급 |
...
| 6~14 | 등급별 골드 획득 | instant | 스킬별 설정 | 일정 공격 횟수마다 골드 획득 |
...
`assets/data/stats.json`의 몬스터 `hp`와 `goldReward`는 모두 **1마리 기준값**입니다.

```text
스테이지 배율 = 1 + (라운드 × 0.01)
개별 몬스터 HP = floor(monster.hp × 스테이지 배율)
개별 처치 골드(내부) = round(monster.goldReward × 스테이지 배율, 소수점 1자리)
개별 처치 골드(화면) = floor(보정된 골드) 정수 표시
실제 수량 = wave.count (일반·보스 공통)
T_HP = 보정된 개별 몬스터 HP × 실제 수량
T_GOLD = 보정된 개별 처치 골드 × 실제 수량
```

스테이지 배율은 R1 `1.01배`부터 R50 `1.50배`까지 적용합니다. 최대 스폰 수량 제한,
초과 수량 HP 보정, `goldReward ÷ count` 보정은 적용하지 않습니다.
`clearGoldBonus`와 골드 획득형 타워 보상은 `T_GOLD`와 별도의 경제 수입입니다.

스테이지 진행 규칙은 몬스터의 보스 여부가 아니라 `waves[].timeAttack` 데이터로 결정됩니다.
`timeAttack: true`인 스테이지는 설정 수량을 모두 처치해야 완료되며, `timeLimit` 안에
모두 처치하지 못하면 게임 오버가 됩니다. `timeAttack: false`인 스테이지는 스폰 완료 후
기존 자동 진행 규칙을 사용합니다.

### 50스테이지 선형 밸런스 곡선

`scripts/monster/rebalance_linear_stages.js`로 다음 데이터 곡선을 재현할 수 있습니다.

| 항목 | 시작 | 종료 | 규칙 |
|---|---:|---:|---|
| 일반 몬스터 기본HP | R1 1,710 | R49 26,460 | R1~7 별도 선형 곡선 후 기존 초중반 곡선 연결 및 ×1.50 |
| 일반 몬스터 수량 | R1 18 | R49 36 | 선형 증가 |
| 일반 몬스터 타입 | small | 반복 | small → large → general 순환 |
| 보스 HP | R8 42,000 | R50 1,260,000 | 선형 기준 HP에 초중반 완화 계수 및 ×0.70 적용 |
| 몬스터 처치 골드 | R1 일반 3.8 / 보스 3.5 | R50 일반 11.1 / 보스 8.2 | 일반 ×1.25, 보스 ×0.91 후 스테이지 배율 |
| 스테이지 클리어 골드 | R1 10 | R50 90 | 모든 스테이지 선형 증가 |
| 스폰 간격(데이터 기준) | R1 1,800ms | R50 1,200ms | 50ms 단위 선형 감소 |

일반 몬스터 웨이브는 데이터 스폰 간격에 `1.25` 배율을 적용합니다(R1 2,250ms,
R50 1,500ms). 보스 웨이브와 DPS 모드 스폰 간격은 변경하지 않습니다.

일반 몬스터는 기존 기준 대비 HP `×1.50`, 처치 골드 `×1.25`로 상향했습니다.
보스는 HP `×0.70`, 처치 골드 `×0.91`로 조정했습니다. 이는 일반 난이도 증가분의
50%를 골드로 환산하고, 보스 난이도 감소분의 30%를 골드 감소로 환산한 기준입니다.

R31~50은 `R31~37 / R38~44 / R45~50`의 3단계로 구분합니다. 마지막 3단계(R45~50)는
현재 적용값 기준으로 몬스터 HP `×1.20`, 처치 골드 `×0.70`을 추가 적용합니다.

보스 처치 골드는 직전 일반 스테이지의 HP 대비 골드 비율을 적용합니다.
`보스 HP × (직전 일반 골드 ÷ 직전 일반 HP)`로 계산하며 소수값을 그대로 누적합니다.
현재는 보스 킬 골드에 리스크 보너스 `+35%`를 적용하고, 최종값을 소수점 1자리까지 저장합니다.

R1~7 개별 HP는 기존 R6~7 수준을 기준으로 `1,140 → 1,320`까지 라운드당 30씩
선형 증가합니다. R8 이후 HP 완화 계수는 R8~15 `0.50`, R16~30 `0.51→0.83`, R31~40 `0.85→1.00`,
R41~50 `1.00`입니다. 초반 500G로 시작한 저운 베이스 타워 모델에서 R30 도달자의
필드 중앙값은 약 9마리이며, 진행 중 최대 필드 중앙값은 16마리, 상위 10%는
약 41마리입니다. 3만 회 모의 실행에서 R15 도달률은 56.46%, R30 도달률은
17.58%였습니다.

기본 보상은 시작 골드를 포함해 약 119.77회 뽑기를 제공합니다. 전체 배치 수용량
490기 기준 24.44%입니다. 현재 뽑기는 기본 타워 27종으로 제한되어 골드 타워의
추가 수입은 발생하지 않습니다. 최종 클리어율은 이 기본 타워 전용 풀을 기준으로
별도 재검증해야 합니다.
...
| 라운드 | 보스 타입 | HP | 색상 |
|---|---|---|---|
| R8 | boss_small (소형) | 60,000 | 🟠 주황 |
| R16 | boss_normal (일반) | 204,000 | 🔴 빨강 |
| R24 | boss_normal (일반) | 471,000 | 🔴 빨강 |
| R32 | boss_large (대형) | 832,000 | 🔵 파랑 |
| R40 | boss_normal (일반) | 1,240,000 | 🔴 빨강 |
| R48 | boss_small (소형) | 1,520,000 | 🟠 주황 |
| R50 | boss_large (대형) | 1,800,000 | 🔵 파랑 |
...
| 타워 | 108종 유닛 능력치 (DPS 자동 계산, 가챠 활성 토글) |
| 스킬 | 14종 스킬 파라미터 (독·골드 파라미터 포함) |
| 몬스터 | 50라운드 1마리 기준 HP·골드, 수량, T_HP, T_GOLD, 타입/보스여부 |
| 스테이지 | 웨이브 구성 |
